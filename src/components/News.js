...
const MAX_NEWSAPI_RESULTS = 100;

const API_BASE = process.env.REACT_APP_API_BASE || "https://news-tape.vercel.app";
const buildUrl = (page, pageSize, category) =>
  `${API_BASE}/api/news?page=${page}&pageSize=${pageSize}&category=${encodeURIComponent(category)}&country=us`;
...
const News = ({ pageSize, category, setProgress, isDarkMode }) => {
  ...
  const updateNews = async (pageToFetch) => {
    setProgress(10);
    const url = buildUrl(pageToFetch, pageSize, category);
    try {
      const response = await fetch(url);
      setProgress(30);
      const data = await response.json();
  ...
  const fetchMoreData = async () => {
    const nextPage = page + 1;
    const url = buildUrl(nextPage, pageSize, category);
    try {
      const response = await fetch(url);
      const data = await response.json();
      const incoming = data.articles || [];
...
News.defaultProps = { pageSize: 8, category: 'general' };
News.propTypes = {
  pageSize: PropTypes.number,
  category: PropTypes.string,
  setProgress: PropTypes.func.isRequired,
